...
but do it in a way where the X axis is varying K
"""
module Tst
...
function problem_factory(problem_name::String, σ²::Real, alpha=1.0, Delta=0.1, seed::UInt32=987; K=-1)
...
opt = (
       ...
       problem_name = "equal", # "poly_K20", "linear_K20", "linear", "linear_K2_custom", "linear_K2"
       K_ary = 2 .^ (2:8),
)
...
#    global algo, problem
    ...
    for i_K in eachindex(opt.K_ary)
        ...
        alpha=NaN; Delta = NaN
        problem = problem_factory(opt.problem_name, opt.σ², alpha, Delta, myseed, K=K)
...
bias = meansqueeze(hatvalue, 1) # n_alphas x n_estimators
...
x = opt.K_ary
i = 1; plot(x, mse[:,i], label=estimator_names[i],linewidth=3)
