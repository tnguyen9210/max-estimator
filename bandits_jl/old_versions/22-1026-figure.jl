...
opt = (T=300, #1000
       ...
       algoseed=789,
       n_trials = 1000, # 200
       algo_name = "ucb", # "ucb", "sr", "uniform"
       problem_name = "linear_K20" #"linear", "linear_K2_custom", "linear_K2"
)
estimator_names = ["avg", "max", "weighted_100", "weightedms_4"]
...
x = problem.K:opt.T
plot(x, ones(length(x)) * .9, color=:black, label="true", linewidth=2)
estimator_names_2 = ["average", "max", "WE", "WE-MS"]

i = 1; plot!(x, me[x,i], label=estimator_names_2[i],linewidth=3)
i = 2; plot!(x, me[x,i], label=estimator_names_2[i],linewidth=3)
i = 3; plot!(x, me[x,i], label=estimator_names_2[i],linewidth=3)
i = 4; plot!(x, me[x,i], label=estimator_names_2[i],linewidth=3, xtickfont=font(18), ytickfont=font(18), legendfont=font(18), xlabel="time", labelfontsize=18)
