...
opt = (σ² = (1.0)^2, 
       dataseed=123,
       algoseed=789,
       n_trials = 50, #1000, # 200
       algo_name = "uniform", # "ucb", # "ucb", "sr", "uniform"
       problem_name = "allbest", #"linear_K20" #"linear", "linear_K2_custom", "linear_K2"
       K_ary = 2 .^ (3:10),
)
estimator_names = ["naiveoracle", "avg", "max", "weighted_100", "weightedms_4", "haver"]
...
#    global algo, problem
    ...
    for i_K in eachindex(opt.K_ary)
        ...
        T = convert(Int64, ceil(K^(1.5))) #  2.0
        for t in 1:T
            i_t = next_arm(algo)
